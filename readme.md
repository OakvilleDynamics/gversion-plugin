...
`gversion-plugin` is a Gradle plugin for auto-generating a version class/file in multiple JVM Languages. Support provided for Java, Kotlin, YAML and Properties.

The class will include information which can only be obtained at compile time, such as build time, Git SHA, and Gradle version.
...
```gradle
plugins {
  id "com.oakvillerobotics.gversion" version "1.10.3"
}
...
`DIRTY` indicates if there are uncommitted versioned files in your repository.

* `0` means it's clean
* `1` means it's dirty
* `-1` means something went wrong with the git command
...
projects that are broken up into many subprojects and take minutes to build, you might want to consider placing
auto generated code in a different separate subproject not depended on by other subprojects or switching to the
...
```gradle
apply plugin: 'com.oakvillerobotics.gversion'
...
buildscript {
    ...
    dependencies {
        classpath "com.oakvillerobotics.gversion:gversion-plugin:$VERSION"
    }
}
```
...
This plugin was originally written by Peter Abeles and has been released under a dual license: Public Domain and MIT.
It has since been modified by Oakville Dynamics for its own usage with the *FIRST* Robotics Competition.

Please use GitHub Issues to post bugs and feature requests.
